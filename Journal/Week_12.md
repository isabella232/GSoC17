...
So, the issue was not with `traverse got undefined, returning`, because this 
is in fact a part of the code that is necessary to exit from the search for 
matching patterns in `/lib/lifecycle/resolve-input.js`. So what I found to be
 the problem with the what I was trying to achieve is that in current API **we 
 cannot reference to files within current working directory that are not 
 generated by bionode-watermill**. This occurs because `matchee function` in 
 the same file just looked for `currentCollection`, which is in fact 
 generated by bionode-watermill, other than that we could not match to 
 filesystem. So, I added a solution that allows that users place some input 
 files within current working directory and bionode-watermill can access them
  **by crawling the current working directory folder**. Check the commit 
  [here](https://github.com/bionode/bionode-watermill/pull/76/commits/98597126c43ede2aa858c05dca6ee24c44bf428c).

So now, lets see what happens with the same type of attempt that we tryed 
before (but first for just one input):
...
// first lets uncompress the gz
const gunzipIt = task({
    input: '*_genomic.fna.gz',  // notice the difference! There is no need to
     // hardcode the file  assuming that it is already in cwd or that it is 
     // putted there by bionode-watermill or some script executed within 
     // watermill.
    output: '*.fna',
    name: 'gunzipIt task'
...
Then we call the pipeline as follows:
...
So the idea is again to run only once `getReference` and the `pipeline` also 
once given that we have only one input (in this case):
